...
.kd-conversation { flex: 1 1 auto; min-height: 0; background: var(--s7-light); padding: var(--s7-pad); user-select: text; }
.kd-agent-main { flex: 1 1 auto; min-height: 0; display: flex; }
.kd-agent-main > .kd-conversation { min-width: 0; }
...
.kd-session-picker { min-height: 0; flex: 1; display: flex; flex-direction: column; }
.kd-launcher-top { flex: 0 0 auto; display: grid; grid-template-columns: calc(var(--s7-icon) * 2) minmax(var(--s7-min-window-width),1fr) auto auto; gap: var(--s7-gap); align-items: center; padding: calc(var(--s7-pad) * 2); border-bottom: 1px solid var(--s7-dark); background: var(--s7-surface); }
...
.kd-agent-search { padding: var(--s7-pad); border-bottom: 1px solid var(--s7-dark); }
.kd-agent-search .s7-input { width: 100%; }
.kd-launcher-actions { display: flex; justify-content: flex-end; flex-wrap: wrap; gap: var(--s7-gap); padding: var(--s7-pad); border-top: 1px solid var(--s7-dark); background: var(--s7-surface); }
