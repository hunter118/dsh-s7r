...
export function SystemWindow({ window, active, baseFontSize, pixelScale, children, onFocus, onClose, onMove, onResize, onZoom, onCollapse }: SystemWindowProps) {
  ...
  const fixedSize = window.resizable === false || window.appId === 'clock' || window.appId === 'puzzle' || window.appId === 'agent-setup'
  return <section
    className="kd-window"
  ...
  >
    <header className="kd-title-bar"
      onDoubleClick={event => { if (!fixedSize && (event.target as HTMLElement).closest('button') === null) onZoom() }}
      onPointerDown={event => {
    ...
    >
      ...
      <span className="kd-title-controls"><button className="kd-collapse" aria-label={collapsed ? 'Expand window' : 'Collapse window'} data-balloon={collapsed ? 'Expand this window' : 'Collapse this window to its title bar'} onClick={event => { event.stopPropagation(); onCollapse() }}>—</button>{fixedSize ? null : <button className="kd-zoom" aria-label="Zoom window" data-balloon="Zoom or restore this window" onClick={event => { event.stopPropagation(); onZoom() }} />}</span>
    </header>
    <div className="kd-window-body">{children}</div>
    {!collapsed && !fixedSize ? <div className="kd-resizer" data-balloon="Drag to resize this window"
      onPointerDown={event => {
