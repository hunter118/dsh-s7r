...
describe('versioned local persistence', () => {
  it('migrates partial or invalid display preferences', () => {
    ...
    expect(migrated).toEqual({ version: 14, resolution: 'compact', baseFontSize: 12, pixelScale: 1, uiAppearance: 'monochrome', filterImages: true, filterPdf: true, filterMode: 'monochrome', wallpaper: 'classic', wallpaperFilterMode: 'grayscale', wallpaperPixelSize: 1, wallpaperFit: 'tile' })
    expect(migrateDisplayPreferences({ version: 2, resolution: 'classic', uiRaster: 'framebuffer' })).toMatchObject({ version: 14, resolution: 'adaptive', baseFontSize: 12, pixelScale: 1, uiAppearance: 'monochrome' })
    ...
    expect(migrateDisplayPreferences({ version: 6, filterImages: false })).toMatchObject({ version: 14, filterImages: false, filterMode: 'monochrome' })
    expect(migrateDisplayPreferences({ version: 7, filterMode: 'grayscale' })).toMatchObject({ version: 14, filterMode: 'grayscale', wallpaper: 'classic' })
    expect(migrateDisplayPreferences({ version: 8, wallpaper: 'cats', wallpaperFilterMode: 'monochrome', wallpaperFit: 'cover' })).toMatchObject({ version: 14, wallpaper: 'cat', wallpaperFilterMode: 'monochrome', wallpaperPixelSize: 1, wallpaperFit: 'cover' })
    ...
    expect(migrateDisplayPreferences({ version: 10, wallpaper: 'cat2' })).toMatchObject({ version: 14, wallpaper: 'cat' })
    expect(migrateDisplayPreferences({ version: 11, wallpaper: 'cat2' })).toMatchObject({ version: 14, wallpaper: 'cat' })
    expect(migrateDisplayPreferences({ version: 12, wallpaper: 'cat', wallpaperPixelSize: 1 })).toMatchObject({ version: 14, wallpaper: 'classic', wallpaperPixelSize: 1 })
    expect(migrateDisplayPreferences({ version: 13, wallpaper: 'cat', wallpaperPixelSize: 1 })).toMatchObject({ version: 14, wallpaper: 'cat', wallpaperPixelSize: 1, uiAppearance: 'monochrome' })
    expect(migrateDisplayPreferences({ version: 14, uiAppearance: 'color' })).toMatchObject({ version: 14, uiAppearance: 'color' })
  })
